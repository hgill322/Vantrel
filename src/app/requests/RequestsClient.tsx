...
import { REQUEST_TYPE_META, REQUEST_STATUS_META, AUTONOMY_META, PLAN_META, fmtDateTime, type AutonomyLevelKey } from "@/lib/constants";
import type { PropertyRequestWithRelations, SignupRequest, PropertyDraft } from "@/lib/types";

export default function RequestsClient({
  initialRequests,
  initialSignupRequests,
}: {
  initialRequests: PropertyRequestWithRelations[];
  ...
  const resolvedSignups = signupRequests.filter((r) => r.status !== "pending");
  ...
  async function reviewSignup(id: string, status: "approved" | "denied") {
    setError("");
    const reviewNote = status === "denied" ? window.prompt("Optional note for the applicant:") ?? "" : "";
    ...
    const updated = await res.json();
    setSignupRequests(signupRequests.map((r) => (r.id === id ? updated : r)));
  }

  return (
    <main className="max-w-4xl mx-auto px-6 py-8">
      <h1 className="text-lg font-semibold text-stone-900 mb-1">Requests</h1>
      <p className="text-sm text-stone-500 mb-6">New landlord applications and property changes waiting on your approval.</p>

      {error && <div className="mb-4 text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{error}</div>}

      <div className="mb-8">
        <h2 className="text-sm font-medium text-stone-700 mb-3">Pending signups ({pendingSignups.length})</h2>
        <div className="bg-white border border-stone-200 rounded-xl divide-y divide-stone-100 shadow-sm">
          {pendingSignups.length === 0 ? (
            ...
            pendingSignups.map((r) => <SignupRow key={r.id} request={r} onApprove={() => reviewSignup(r.id, "approved")} onDeny={() => reviewSignup(r.id, "denied")} />)
          )}
        </div>
      </div>

      <div className="mb-8">
        <h2 className="text-sm font-medium text-stone-700 mb-3">Pending property requests ({pending.length})</h2>
        <div className="bg-white border border-stone-200 rounded-xl divide-y divide-stone-100 shadow-sm">
          {pending.length === 0 ? (
            <div className="text-sm text-stone-400 text-center py-10">Nothing waiting on you.</div>
          ) : (
      ...
      <div className="mb-8">
        ...
        <div className="bg-white border border-stone-200 rounded-xl divide-y divide-stone-100 shadow-sm">
          {resolved.length === 0 && resolvedSignups.length === 0 ? (
            <div className="text-sm text-stone-400 text-center py-10">No reviewed requests yet.</div>
          ) : (
            <>
              {resolvedSignups.map((r) => <SignupRow key={r.id} request={r} />)}
...
function SignupRow({ request, onApprove, onDeny }: { request: SignupRequest; onApprove?: () => void; onDeny?: () => void }) {
  ...
  const plan = PLAN_META[request.planKey as AutonomyLevelKey];

  return (
    <div className="px-4 py-3 flex items-start justify-between gap-4">
      <div className="min-w-0">
        <div className="text-sm font-medium text-stone-900">{request.name} · {request.email}</div>
        <div className="text-xs text-stone-500 mt-0.5">
          {plan?.name ?? request.planKey} &middot; {properties.length} propert{properties.length === 1 ? "y" : "ies"}
        </div>
