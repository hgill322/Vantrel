...
export default async function RequestsPage() {
  ...
  const pendingRequestCount =
    requests.filter((r) => r.status === "pending").length + signupRequests.filter((r) => r.status === "pending").length;

  return (
    <>
      <StaffHeader issues={issues} role={session.role} pendingRequestCount={pendingRequestCount} />
      <RequestsClient initialRequests={requests} initialSignupRequests={signupRequests} />
    </>
  );
}
